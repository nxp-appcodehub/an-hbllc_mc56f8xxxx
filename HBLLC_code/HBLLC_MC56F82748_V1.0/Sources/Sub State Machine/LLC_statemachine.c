...
/* PWM mode is implemented by changing the dead time. In PFM mode, the min dead time is used.*/	
    ...
    PWMA->SM2VAL1 = (u16HalfTs<<1)-1;
	PWMA->SM2VAL2 = u16QuaterTs+u16Deadtime;
	PWMA->SM2VAL3 = u16HalfTs+u16QuaterTs;
	if(u16Deadtime > u16QuaterTs) PWMA->SM2VAL4 = u16Deadtime-u16QuaterTs;
	else PWMA->SM2VAL4 =  u16HalfTs+u16QuaterTs+u16Deadtime;
	PWMA->SM2VAL5 =  u16QuaterTs;
#endif	
	...
	PWMA_SM2VAL1 = (u16HalfTs<<1)-1;
	PWMA_SM2VAL2 = u16QuaterTs+u16Deadtime;
	PWMA_SM2FRACVAL2 = (u16FracVal<<11);
	PWMA_SM2VAL3 = u16HalfTs+u16QuaterTs;
	if(u16Deadtime > u16QuaterTs) PWMA_SM2VAL4 = u16Deadtime-u16QuaterTs;
	else  PWMA_SM2VAL4 = u16HalfTs+u16QuaterTs+u16Deadtime;
	PWMA_SM2FRACVAL4 = (u16FracVal<<11);
	PWMA_SM2VAL5 = u16QuaterTs;
#endif
...
#if SR_ENABLE
	PWMA_SM1VAL1 = (u16HalfTs<<1)-1;
	PWMA_SM1VAL2 = u16QuaterTs+u16Deadtime;
	PWMA_SM1VAL3 = u16HalfTs+u16QuaterTs+SR_OFFDELAY;
	if(u16Deadtime > u16QuaterTs) PWMA_SM1VAL4 = u16Deadtime-u16QuaterTs;
	else   PWMA_SM1VAL4 = u16HalfTs+u16QuaterTs+u16Deadtime;
	PWMA_SM1VAL5 = u16QuaterTs+SR_OFFDELAY;
#endif

#if CTRL_FRE_CHANGE_ENABLE	
	if(f16HalfTs <= (gsLLC_Drive.sPeriodCtrl.f16CtrlFreqRatioLim1))
	{
		PWMA_SM0VAL1 = (u16HalfTs*6)-1; // the switching frequency is three times the control time
	}
	else if(f16HalfTs <= gsLLC_Drive.sPeriodCtrl.f16CtrlFreqRatioLim2)
	{
	    PWMA_SM0VAL1 = (u16HalfTs<<2)-1; // the switching frequency is twice the control frequency
	}
	else
	{
	    PWMA_SM0VAL1 = (u16HalfTs<<1)-1; // the switching frequency equals the control frequency
	}
#endif 
	/* load new values to PWM registers */
	PWMA_MCTRL |= PWM_MCTRL_LDOK(1); /* LDOK=1,load enable */ 
}

...
#define LOAD_ENABLE       GPIOC_DR &= ~0x10  /* connect load */
#define LOAD_DISABLE      GPIOC_DR |= 0x10   /* disconnect load */

#define ADC_RDY_08        ADC_RDY & 0x101
#define ADC_RDY_19        ADC_RDY & 0x202

#define PWMA_RUN          PWMA_MCTRL |= PWM_MCTRL_RUN(7)
#define PWMA_STOP         PWMA_MCTRL &= ~PWM_MCTRL_RUN(7)

#define HW_IPRIM_OVER()               PWMA_FSTS0 & PWM_FSTS_FFLAG(1)
#define HW_FAULT_FLAG_CLEAR()  	      PWMA_FSTS0 |= PWM_FSTS_FFLAG(1)

#define PIT_RUN           PIT0_CTRL |= PIT_CTRL_CNT_EN_MASK
#define PIT_STOP          PIT0_CTRL &= ~PIT_CTRL_CNT_EN_MASK

#define ENABLE_PWM_OUTPUT       { AOI_BFCRT012 |= AOI_BFCRT01_PT0_BC_MASK;\
	                              AOI_BFCRT013 |= AOI_BFCRT01_PT0_BC_MASK;}
#define DISABLE_PWM_OUTPUT      { AOI_BFCRT012 &= ~AOI_BFCRT01_PT0_BC_MASK;\
	                              AOI_BFCRT013 &= ~AOI_BFCRT01_PT0_BC_MASK;}
#define ENABLE_SR     { AOI_BFCRT010 |= AOI_BFCRT01_PT1_AC_MASK;\
	                    AOI_BFCRT011 |= AOI_BFCRT01_PT1_AC_MASK;}      
#define DISABLE_SR    { AOI_BFCRT010 &= ~AOI_BFCRT01_PT1_AC_MASK;\
	                    AOI_BFCRT011 &= ~AOI_BFCRT01_PT1_AC_MASK;} 

#define TP30_UP      GPIOE_DR |= 0x40
#define TP30_DOWN    GPIOE_DR &= ~0x40
#define TP31_UP      GPIOE_DR |= 0x80
#define TP31_DOWN    GPIOE_DR &= ~0x80

#define LLC_BOARD_LED3_ON   GPIOC_DR |= 0x01
#define LLC_BOARD_LED3_OFF  GPIOC_DR &= ~0x01
#define LLC_BOARD_LED4_ON   GPIOF_DR |= 0x40
#define LLC_BOARD_LED4_OFF  GPIOF_DR &= ~0x40
